...
class Parser(argparse.ArgumentParser):
    ...
    def _load_conf_file(self, conf):
        ...
        if isinstance(self._conf, str):
            ...
            self._log.debug("Trying to read a configuration file.")
            try:
...
def check_overwrite(filename, force=False, auto=False, append=None):
    """Checks whether the path of the filename exists. If it does, asks to overwrite or change the name of the file.

    :param str filename: name of the file to be created or overwritten.
    :param bool force: whether to force the overwrite if necessary without asking.
    :param bool auto: whether to automatically create a new file with an appended string.
    :param None or str or bytes append: Optional appendix to be added to the filename in case it already exists.
    :return: filename or new file name.
    :rtype: str
    """
    ...
    while path_exists(filename):
        ...
        if force:
            ...
            filename = name + (f'_{file_v:02.0f}' if append is None else append) + ext
            if append:
                append = append + f'_{file_v:02.0f}'
            file_v += 1
        else:
            if yesno_question('Do you want to overwrite %s?' % filename):
                break
            else:
                filename = input('Write a new name for the file: ')
...
def fmt_latex_table(tables, caption=('Caption of the table',), label=('tab:table1', ), **kwargs):
    ...
    if kwargs.get('save', False):
        for t, latex_table in enumerate(latex_tables):
            ...
            filename = check_overwrite(filename, auto=True, force=kwargs.pop('overwrite', False)) + '.tex'
