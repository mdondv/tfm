...
import re   # regular expressions library (for searching patterns in strings)
...
def round_general(f, up_down='up', even_odd='even', dtype=np.int64):
    """ Function that rounds up or down the array 'f' to the closest even or odd number
...
def normalize_dataframe(data, copy=False, clean=True, **kwargs):
    ...
    :param kwargs: Additional keyword arguments, such as 'normalize_labels' or 'clean_labels'.
...
def get_all_files(data_dir='./results'):
    # Load data
    ...
    for fp in file_list:
        ...
        if ext == '.csv':
            ...
            filename.append(data_dir + fp)
            dfs.append(pd.read_csv(data_dir + fp))
            dfs[-1].attrs = dict(datafile=(data_dir + fn))
...
def compute_estimation(dataframe, nbins=41, xlabel='average_circ', ylabel='estim', **kwargs):
    ...
    x_avg = gr1[xlabel].apply(circmean, low=-180, high=180)
...
def compute_pm_reg(dataframe, bins=41, lim=45, compute=False, save=True, plabel='pright', ylabel='binchoice_phase',
    ...
    if plabel not in dataframe.columns or compute:
        ...
        np.save(dataframe.attrs['datafile'] + '_pm_reg', dict(fit), allow_pickle=True)
...
def compute_pm_data(dataframe, bins=21, lim=45.0, sample_size=0.1, compute_errorbars=False, xlabel='average_circ',
                    ylabel='estim'):
    ...
    :param str xlabel: column label corresponding to the independent variable.
    :param str ylabel: column label corresponding to the binary outcome.
    ...
    # We define the categories for the data points and the model points/line
    dataframe['bin'] = equalbins(dataframe[xlabel], bins)
...
def get_amp_eq_stats(i0s=(0.02, 0.05, 0.08), **kwargs):
    ...
    for i0 in i0s:
        ...
        try:
            data = np.load(dfile + '.npy', allow_pickle=True)
            conf = data['conf']
        except IndexError:
            # noinspection PyTypeChecker
...
def compute_sqr_error(filename, betas, k=1):
    ...
    :param pd.DataFrame filename: name of the file that contains the trial data.
...
def get_sqr_errors(**kwargs):
    ...
    directory = kwargs.get('data_dir', './results/variable_frames/')
    ...
    stats_file = kwargs.get('stats_file', f"./results/integration_stats_frames-{nframes}_t-{tmax:.1f}_new.csv")
    ...
    if len(new_sel) != 0:
        ...
        for n, (k, row) in enumerate(new_sel.iterrows()):
            ...
            df.attrs['datafile'] = row.filename[:-4]
...
# ##############################################################################################
@apply_custom_style
def plot_trial(r_data, dataframe, figsize=np.array([5.0, 5.5]) * 1.2, save=False, **kwargs):
    ...
    # G1
    (top1, bottom1) = (0.95, 0.45)
    gs1.update(left=left, right=right_1, top=top1, bottom=bottom1, hspace=hspace, wspace=wspace)
    # G2
    (top2, bottom2) = (bottom1 - 0.1, 0.35)
    gs2.update(left=left, right=right_1, top=top2, bottom=bottom2, hspace=hspace, wspace=wspace)
    # G3
    (top3, bottom3) = (bottom1 - 0.12, 0.08)
    gs3.update(left=left, right=0.64, top=top3, bottom=bottom3, hspace=hspace, wspace=wspace)
    # G4
    (top4, bottom4) = (top3, bottom3)
    gs4.update(left=0.7, right=0.97, top=top4, bottom=bottom4, hspace=hspace, wspace=wspace)
    # Gbar
    ...
    ax_amp = fig.add_subplot(gs1[0, 0])
    ax_ph = fig.add_subplot(gs1[1, 0], sharex=ax_amp)
    ax_c = fig.add_subplot(gs1[2, 0], sharex=ax_ph)

    ax_p1 = fig.add_subplot(gs3[0, 0], sharey=ax_amp)
    ax_p2 = fig.add_subplot(gs3[0, 1], sharey=ax_amp, sharex=ax_p1)
    ax_p3 = fig.add_subplot(gs4[0, 0], sharey=ax_amp, sharex=ax_p1)

    mod_axes([ax_amp, ax_ph, ax_p1, ax_p2, ax_p3])
    ...
    ax_amp.plot(tpoints, amp, label=r'$r(\psi, t)$')
    ax_amp.plot(tpoints, ampm, label=r'$r(\psi+\pi, t)$')
    r_avg = r.mean(axis=1)
    r_std = r.std(axis=1)
    ax_amp, mp, ebmp = plot_error_filled(tpoints, r_avg, r_std, ax=ax_amp)
    mp.set_label(r'$\left\langle r(\theta, t)\right\rangle_\theta$')
    ax_amp.legend(frameon=False, fontsize=8)
    ...
    ax_ph.bar(stim_times, stim_phases, align='edge', width=cue * 1.0, lw=1.0, ec='cornflowerblue',
              fc=stim_color, label=r'$\theta_i^{\rm stim}$')
    ax_ph.plot(tpoints, phase, color='black', linewidth=1.5, label=r'$\psi(t)$')
    ...
    bump = ax_c.pcolormesh(tpoints, theta, r.T, vmin=0)
    bump.set_edgecolor('face')
    ax_c.set_xlabel('Time (s)')
    ax_c.set_ylabel(r'$\theta\ (^\circ)$', labelpad=-1)
    ax_c.set_ylim(-180, 180)
    ax_c.set_yticks([-180, -90, 0, 90, 180])
    ax_c.plot(tpoints, phase, color='black', linewidth=1.5, label=r'$\psi(t)$')
    ...
    # Firing rate profiles at different time points (0.05, 0.250, 0.750)
    cmap = plt.cm.get_cmap('Reds')
    norm = MidpointNormalize(midpoint=0.5, vmin=-0.5, vmax=1.2)
    idx_times = (np.abs(np.tile(tpoints, (4, 1)).T - np.array([0.05, 0.50, 1.00, 2.00]))).argmin(axis=0)
    for idx in idx_times[0:3]:
        ax_p1.plot(theta, r[idx], color=cmap(norm(tpoints[idx])), label=r'$%.2f$ s' % tpoints[idx], linewidth=0.5)
        ax_c.plot(tpoints[idx], 200, 'v', markersize=6, clip_on=False, color=cmap(norm(tpoints[idx])))

    ax_p1.set_xlim(-180, 180)
    ax_p1.set_xticks([-180, -90, 0, 90, 180])
    ax_p1.legend(frameon=False, fontsize=6, loc='upper right')
    ax_p1.set_xlabel(r'$\theta\ (^\circ)$')
    ax_p1.set_ylabel('Firing Rate (Hz)', labelpad=5)

    # Mean Firing rate profiles (aligned, accross time), (not aligned, across time), (2.00 or 2.05)
    r_avg_aligned = r_a.mean(axis=0)
    r_std_aligned = r_a.std(axis=0)
    r_avg_not_aligned = r.mean(axis=0)
    r_std_not_aligned = r.std(axis=0)

    ax_p2, paa, ebaa = plot_error_filled(theta, r_avg_aligned, r_std_aligned, ax=ax_p2)
    paa.set_label(r'$\left\langle r(\theta, t)\right\rangle_t$, aligned')
    ax_p2, pana, ebana = plot_error_filled(theta, r_avg_not_aligned, r_std_not_aligned, ax=ax_p2)
    pana.set_label(r'$\left\langle r(\theta, t)\right\rangle_t$, not aligned')
    ax_p2.legend(frameon=False, fontsize=6, loc='upper left')
    ax_p2.set_xlabel(r'$\theta\ (^\circ)$')
    plt.setp(ax_p2.get_yticklabels(), visible=False)

    # Comparison with trial average results (averages across trials: aligned and not aligned at the end of the trial)
    # Identify trials from this category
    if len(dataframe) > 1:
        category_trials = dataframe.loc[dataframe.category == data_trial.category].index.to_numpy(dtype=int)

        r_cat_avg_aligned = r_aligned[idx_times[-1], category_trials].mean(axis=0)  # All trials
        r_cat_std_aligned = r_aligned[idx_times[-1], category_trials].std(axis=0)  # All trials
        r_cat_avg_not_aligned = r_data[idx_times[-1], category_trials].mean(axis=0)  # All trials
        r_cat_std_not_aligned = r_data[idx_times[-1], category_trials].std(axis=0)  # All trials

        ax_p3, p3aac, eb3aac = plot_error_filled(theta, r_cat_avg_aligned, r_cat_std_aligned, ax=ax_p3)
        p3aac.set_label(r'$\bar{r}$, aligned')
        ax_p3, p3anac, eb3anac = plot_error_filled(theta, r_cat_avg_not_aligned, r_cat_std_not_aligned, ax=ax_p3)
        p3anac.set_label(r'$\bar{r}$, not aligned')

    realtrial = dataframe.iloc[trial]['index']
    ax_p3.plot(theta, r_a[idx_times[-1]], label=r'$r_{%d}$' % (realtrial + 1), color='black', alpha=0.4, linewidth=0.4)
    ax_p3.legend(frameon=False, fontsize=6, loc='upper left')
    ax_p3.set_xlabel(r'$\theta\ (^\circ)$')
    plt.setp(ax_p3.get_yticklabels(), visible=False)

    # Trial parameters, Axes Labels and panel labels
    fig.text(right_1, 1, r'Trial %d. Category: $%d^\circ$' % (realtrial + 1, category), ha='right', va='top')
    ax_p1.text(0.99, 1.01, r'Snapshots, $r\left(\theta, t_0\right)$', ha='right', va='bottom',
               transform=ax_p1.transAxes,
               fontsize=8)
    ax_p2.text(0.99, 1.01, r'Averages', ha='right', va='bottom', transform=ax_p2.transAxes, fontsize=8)
    ax_p3.text(0.99, 1.01, r'Trial average at $t_0=2$ s',
               ha='right', va='bottom', transform=ax_p3.transAxes, fontsize=8)

    panel_labels = ['A', 'B', 'C', 'D', 'E', 'F']
    label_locations = [(0, 1.0), (0, 0.78), (0, 0.62), (0, 0.36), (0.37, 0.36), (0.68, 0.36)]
    for (x, y), lbl in zip(label_locations, panel_labels):
        fig.text(x, y, r'\rm{%s}' % lbl, ha='left', va='top')
    ...
    ax_p1.xaxis.set_major_formatter(FormatStrFormatter('%d'))

    # rectspan(cue, cue + dcue, [ax_amp, ax_ph], color="gray", linewidth=0.5, alpha=0.2)
...
def plot_temp_integ(i0s, sigma, lbl_fmt="x[0-9][0-9]?_norm", save=False, **kwargs):
    ...
    bump = 'bump' if kwargs.pop('bump', False) else 'nobump'
    if bump == 'bump':
        titles = [r"\rm{Recency}", r"\rm{Recency}", r"\rm{Uniform}"]
    else:
        titles = [r"\rm{Recency}", r"\rm{Uniform}", r"\rm{Primacy}"]
...
def plot_bump_vs_nobump(i0s, sigma, lbl_fmt="x[0-9][0-9]?_norm", save=False, **kwargs):
    ...
    mylog = kwargs.pop('mylog', MyLog(10))
    ntrials = kwargs.get('ntrials', 16000)
    titles = [r"\noindent\rm{No-bump condition}\\\rm{Uniform}",
              r"\noindent\rm{Bump condition}\\\rm{Uniform}"]
...
def plot_trial_full(r_data, s_data, d_data, stimulus, dataframe, figsize=np.array([9.0, 6.0]) * 1.2, save=False,
    ...
    ax_st.bar(stim_times, stim_phases, align='edge', width=cue * 1.0, lw=1.0, ec='cornflowerblue',
              fc=stim_color, label=r'$\phi_i^{\rm stim}$')
