...
"""
Functions to generate trials for the post-decisional bias task (confirmation bias)
"""

import math 

def pdb_new_stimuli(num_trials, categories=(0, ), **kwargs):
    """Function that generates the stimuli for the post-choice bias task. Returns a DataFrame containing the design matrix.
    ...
    if list(categories) == list((0,)):
        categories = np.linspace(-20, 20, 5)
    if not isinstance(categories, (list, np.ndarray)):
        categories = np.array(categories)

    # Select the stimuli orientations    
    x1i = np.random.randint(low=0, high=len(categories), size=num_trials)
    x2i = np.random.randint(low=0, high=len(categories), size=num_trials)

    x1i = np.repeat(math.floor(categories.size / 2), x1i.size)
    # x2i = np.arange(categories.size - 1)

    x1 = np.take(categories, x1i)
    x2 = np.take(categories, x2i)

    avg = (x1 + x2) / 2.0
    choice = (x1 > 0) 
    choice[x1 == 0] = np.nan

    choice_trials = np.ones(num_trials) 
    choice_trials[0:num_trials // 2] = 0  # set half of the total trials to be no-choice trials
    np.random.shuffle(choice_trials)
    subj = np.ones(num_trials)

    stim_dict = dict(x1i=x1i, x2i=x2i, x1=x1, x2=x2, average=avg, bincorrect=choice, do_choice=choice_trials,
                     category=avg, subj=subj)
    columns = stim_dict.keys()
    df = pd.DataFrame(stim_dict, columns=columns)
    if kwargs.get('save_stim', False):
        df.to_csv('./obj/Stim_pdb_df_%d.csv' % num_trials)
        df.to_pickle('./obj/Stim_pdb_df_%d.npy' % num_trials)
